...
const EditProjectModal: FC = () => {
  ...
  const { closeForm, setProjectType, setAreaProject, setDesc, setHabilitado, setLink, postData } = useActions(
    adminProjectLogic,
  );

  const { projectTypes } = useValues(siteDataLogic);

  const setProjectArea = (isConcurso: boolean) => {
    if (isConcurso) {
      setAreaProject(ProjectType.concurso);
      return;
    }
    setAreaProject(ProjectType.feria);
  };
  ...
  return (
    <Modal open={showForm} onClose={closeForm}>
      <div>
        ...
        {form.area !== ProjectType.talk && (
          <>
            <SwitchC
              label="Participa en Concurso?"
              active={form.areaProyecto === ProjectType.concurso}
              setActive={setProjectArea}
              disabled={isLoading}
            />
            <SelectOption
